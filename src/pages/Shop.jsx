...
import { IconCar, IconFilter, IconX, IconCheck } from '../components/icons'
...
export default function Shop() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      ...
      list = list.filter((p) => {
        ...
        if (
          ...
          p.partNo.toLowerCase().includes(needle) ||
        ...
        return interchangesFor(p.partNo).some(
          (x) =>
            x.oem.toLowerCase().includes(needle) ||
    ...
    if (vehicleId) {
      ...
      list = list.filter(
        (p) =>
          p.fits.includes(vehicleId) ||
          (fitVehicle &&
            p.fits.some((f) => allVehicles.find((x) => x.id === f)?.make === fitVehicle.make)),
      )
    }
  ...
  const hasFilters =
    cat || q || vehicleId || selectedBrands.length || maxPrice || inStock
    ...
    <div className="shop">
      <div className="shop-head">
        <div>
          ...
          <p className="shop-count">
            ...
            {vehicle && (
              <>
                {' '}for <strong>{vehicle.make} {vehicle.model} ({vehicle.years})</strong>
              </>
            )}
          </p>
        </div>
        <div className="shop-actions">
          <button className="btn filter-toggle" onClick={() => setShowFilters((s) => !s)}>
            <IconFilter width="17" height="17" /> Filters
      ...
      {fitmentOpen && (
        <div className="fitment-banner card">
          <span className="fitment-banner-icon">
          ...
          <div className="fitment-banner-copy">
            ...
            <span>
              {vehicle
                ? `Showing parts verified for ${vehicle.make} ${vehicle.model} (${vehicle.engine}).`
                : 'Pick your make and model to see only verified-compatible parts.'}
            </span>
          </div>
          <select
            className="select fitment-select"
            value={vehicleId}
            onChange={(e) => {
              setParam('vehicle', e.target.value)
              setParam('fitment', '1')
            }}
      ...
      <div className="shop-layout">
        <aside className={`filters ${showFilters ? 'filters-open' : ''}`}>
            ...
            {categories.map((c) => (
              <label key={c.id} className="frow">
                <input
                  ...
                  name="cat"
                  checked={cat === c.id}
                  onChange={() => setParam('cat', c.id === cat ? '' : c.id)}
                />
                <span>{c.short}</span>
          ...
          <div className="fgroup">
            <span className="fgroup-title">Vehicle fitment</span>
            <select
              className="select"
              value={vehicleId}
              onChange={(e) => {
                setParam('vehicle', e.target.value)
                setParam('fitment', e.target.value ? '1' : '')
              }}
          ...
          <div className="fgroup">
            ...
            <div className="brand-chips">
              {brands.map((b) => (
                <button
                  key={b}
                  className={`chip ${selectedBrands.includes(b) ? 'chip-active' : ''}`}
          ...
          <div className="fgroup">
            <span className="fgroup-title">Max retail price</span>
            <select
              className="select"
              value={maxPrice}
        ...
        <div className="shop-grid-wrap">
          ...
          {filtered.length === 0 && ready ? (
            <div className="empty-state card">
              ...
              <p>Try clearing a few filters or searching a different part number.</p>
